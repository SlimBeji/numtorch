...
from numtorch.layers import SigmoidActivation
from numtorch.utils.context import eval


def test_constructor():
    layer = SigmoidActivation()
    assert layer.name == "SigmoidActivation"
...
def test_forward():
    ...
    vector_1 = np.array([0, np.log(2)])
    expected_1 = np.array([0.5, 2 / 3])
    ...
    vector_2 = np.array([np.log(0.5), np.log(0.25)])
    expected_2 = np.array([1 / 3, 0.2])
...
def test_grad_methods():
    ...
    # First Test for vector
    vector_1 = np.array([0, np.log(2)])
    expected_1 = np.array([[0.25, 0], [0, 2 / 9]])
    layer(vector_1)
    ...
    # Second Test for vector
    vector_2 = np.array([np.log(0.5), np.log(0.25)])
    expected_2 = np.array([[2 / 9, 0], [0, 0.16]])
    layer(vector_2)
...
def test_eval_mode():
    ...
    # Testing vector
    vector = np.array([0, np.log(2)])
    expected = np.array([0.5, 2 / 3])
